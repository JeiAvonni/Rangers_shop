...
def signin():
    ...
    # Quary to find the user with the same email
        user = User.query.filter(User.email == email).first()


        if user and check_password_hash(user.password, password): 
            # This is where we are using our UserMixin class & load_user() function to load user
    ...
    # If its a 'GET' request we need to display thisscreen to the user
